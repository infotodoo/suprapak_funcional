...
class ActivityDataRejected(models.TransientModel):
    _name = "data.rejected"
    _description = 'wizard create revision buttom'

    message = fields.Text('observations')
    users_ids = fields.Many2many('res.users', 'users_rejected_rel', 'rejected_id', 'user_id', 'Users')

    def action_create_rejected(self):
        for record in self:
