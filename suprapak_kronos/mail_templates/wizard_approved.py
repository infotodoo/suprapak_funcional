...
class ActivityDataApproved(models.TransientModel):
    _name = "data.approved"
    _description = 'wizard create revision buttom'

    message = fields.Text('observations')
    users_ids = fields.Many2many('res.users', 'users_approved_rel', 'approved_id', 'user_id', 'Users')

    def action_create_approved(self):
        for record in self:
