...
class ActivityDataRejectedT(models.TransientModel):
    _name = "data.rejected.t"
    _description = 'wizard create revision buttom'

    message = fields.Text('observations')
    users_ids = fields.Many2many('res.users', 'users_rejected_t_rel', 'rejected_t_id', 'user_id', 'Users')

    def action_create_rejected_t(self):
        for record in self:
