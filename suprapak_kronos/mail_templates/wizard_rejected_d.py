...
class ActivityDataRejectedD(models.TransientModel):
    _name = "data.rejected.d"
    _description = 'wizard create revision buttom'

    message = fields.Text('observations')
    users_ids = fields.Many2many('res.users', 'users_rejected_d_rel', 'rejected_d_id', 'user_id', 'Users')

    def action_create_rejected_d(self):
        for record in self:
